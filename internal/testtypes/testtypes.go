...
// Fill registers ifname, ipv4, vlan, asn, and text and panics if fixture construction fails.
func Fill(r *value.Registry) {
	for _, t := range []value.Type{
		...
		{Name: "text", Pattern: `.*`}, // matches the empty string, unlike the builtin rest
	} {
